---
layout: page
title: Ứng dụng
subtitle: Các ứng dụng được phát triển bởi FreeGameStudio
permalink: /apps/
---

<div class="apps-grid">

  <div class="app-card">
    <div class="app-card-header">
      <div class="app-icon app-icon--manga">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="48" height="48"><rect width="64" height="64" rx="14" fill="#E8450A"/><rect x="12" y="10" width="26" height="34" rx="3" fill="#fff" opacity=".95"/><rect x="16" y="16" width="18" height="2" rx="1" fill="#E8450A"/><rect x="16" y="21" width="14" height="2" rx="1" fill="#E8450A" opacity=".6"/><rect x="16" y="26" width="16" height="2" rx="1" fill="#E8450A" opacity=".6"/><rect x="26" y="20" width="26" height="34" rx="3" fill="#fff" opacity=".9"/><rect x="30" y="26" width="18" height="2" rx="1" fill="#1a1a1a" opacity=".4"/><rect x="30" y="31" width="14" height="2" rx="1" fill="#1a1a1a" opacity=".3"/><rect x="30" y="36" width="16" height="2" rx="1" fill="#1a1a1a" opacity=".3"/></svg>
      </div>
      <div class="app-info">
        <h3 class="app-name">Read Manga Online</h3>
        <p class="app-dev">FreeGameStudio</p>
        <span class="app-badge">Miễn phí</span>
      </div>
    </div>
    <p class="app-desc">Đọc manga trực tuyến và ngoại tuyến. Cập nhật chapter mới liên tục, hỗ trợ tiếng Việt và tiếng Anh.</p>
    <div class="app-stores">
      <a href="https://apkpure.com/p/com.dongdoihy.appmangavn" class="store-badge" target="_blank" rel="noopener" title="Tải trên APKPure">
        <img src="{{ '/assets/img/badge-apkpure.svg' | relative_url }}" alt="Get it on APKPure" width="160" height="48">
      </a>
      <a href="https://www.amazon.com/gp/product/B0FH7DJ7HY" class="store-badge" target="_blank" rel="noopener" title="Tải trên Amazon Appstore">
        <img src="{{ '/assets/img/badge-amazon.svg' | relative_url }}" alt="Get it on Amazon Appstore" width="160" height="48">
      </a>
    </div>
  </div>

  <div class="app-card">
    <div class="app-card-header">
      <div class="app-icon app-icon--comic">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="48" height="48"><rect width="64" height="64" rx="14" fill="#1C3A6E"/><circle cx="32" cy="28" r="14" fill="#E8C840" opacity=".9"/><text x="32" y="33" font-family="Arial,sans-serif" font-size="14" font-weight="900" fill="#1C3A6E" text-anchor="middle">DC</text><path d="M14 44 Q32 38 50 44" stroke="#E8C840" stroke-width="2" fill="none" stroke-linecap="round"/><text x="32" y="56" font-family="Arial,sans-serif" font-size="7" font-weight="bold" fill="#fff" text-anchor="middle" letter-spacing="1">MARVEL</text></svg>
      </div>
      <div class="app-info">
        <h3 class="app-name">Comic DC - Marvel</h3>
        <p class="app-dev">FreeGameStudio</p>
        <span class="app-badge">Miễn phí</span>
      </div>
    </div>
    <p class="app-desc">Đọc truyện tranh DC và Marvel trực tuyến & ngoại tuyến. Kho nội dung phong phú từ hai vũ trụ siêu anh hùng lớn nhất thế giới.</p>
    <div class="app-stores">
      <a href="https://www.amazon.com/gp/product/B0FHG8C3TM" class="store-badge" target="_blank" rel="noopener" title="Tải trên Amazon Appstore">
        <img src="{{ '/assets/img/badge-amazon.svg' | relative_url }}" alt="Get it on Amazon Appstore" width="160" height="48">
      </a>
    </div>
  </div>

</div>
